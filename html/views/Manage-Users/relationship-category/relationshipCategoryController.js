angular.module('app', ['ng-fusioncharts'])
    .controller('customerRelationCategoryCtrl', function ($state, $rootScope,$localStorage,$translate, $scope, $uibModal, relationCategoryService,userService, $location) {
     ...
     .controller('providerRelationCategoryCtrl', function ($state, $rootScope, $scope, $uibModal, relationCategoryService,userService, $location) {
        $scope.myDataSource = {};
        $scope.displayCount = $rootScope.userPagination;
        $scope.callServer = function callServer(tableState) {
            $rootScope.displayName = '';
        ...
        $scope.updateCategory = function (row) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($uibModalInstance, $scope, item) {
                    ...
                    var params = {};
                    $scope.updateprovider = function (category) {
                        ...
                        if (typeof category.id_provider_relationship_category != 'undefined' && ((isNaN(category.id_provider_relationship_category) === false && category.id_provider_relationship_category > 0) || (isNaN(category.id_provider_relationship_category) === true && category.id_provider_relationship_category.length > 0))) {
                            ...
                            relationCategoryService.providerupdate(params).then(function (result) {
                                if (result.status) {
                                    ...
                                    var obj = {};
                                    obj.action_name = 'update';
                                    obj.action_description = 'update$$provider category$$(' + item.relationship_category_name + ')';
                                    obj.module_type = $state.current.activeLink;
                                    obj.action_url = $location.$$absUrl;
                                    $rootScope.confirmNavigationForSubmit(obj);
                                    $scope.cancel();
        ...
        $scope.callServer2 = function callServer2(tableState2) {
            ...
            var pagination = tableState2.pagination;
            tableState2.customer_id = $scope.user1.customer_id;
            relationCategoryService.providerAdditionalCategoriesList(tableState2).then(function (result) {
                $scope.newList = result.data.data;
                $scope.displayCount = $rootScope.userPagination;
                $scope.totalRecords2 = result.data.total_records;
                tableState2.pagination.numberOfPages =  Math.ceil(result.data.total_records / $rootScope.userPagination);
                    ...
                    $scope.updateprovider = function (category) {
                        ...
                        if (item) {
                            if (typeof category.id_provider_relationship_category != 'undefined' && ((isNaN(category.id_provider_relationship_category) === false && category.id_provider_relationship_category > 0) || (isNaN(category.id_provider_relationship_category) === true && category.id_provider_relationship_category.length > 0))) {
                                ...
                                params.customer_id = $scope.user1.customer_id;
                                params.relationship_category_name = category.relationship_category_name;
                                params.id_provider_relationship_category_language = category.id_provider_relationship_category_language;
                                params.provider_relationship_category_status = category.provider_relationship_category_status;
                                params.id_provider_relationship_category = category.id_provider_relationship_category;
                                relationCategoryService.providerupdateSub(params).then(function (result) {
                                    if (result.status) {
                                        $rootScope.toast('Success', result.message);
                                        var obj = {};
                                        obj.action_name = 'update';
                            ...
                            params.customer_id = $scope.user1.customer_id;
                            params.relationship_category_name = category.relationship_category_name;
                            relationCategoryService.addProviderSub(params).then(function (result) {
                                if (result.status) {
                                    $rootScope.toast('Success', result.message);
                                    var obj = {};
                                    obj.action_name = 'create';
