...
abstract class REST_Controller extends CI_Controller {
    ...
    protected $http_status_codes = [
        ...
        self::HTTP_INTERNAL_SERVER_ERROR => 'Oops! Something went wrong.',
    ...
    public function __construct($config = 'rest')
    {
        ...
        //print_r($userDetails->language_iso_code);exit;
        ...
        if ($this->auth_override === FALSE && !($this->config->item('rest_enable_keys') && $this->_allow === TRUE))
        {
            ...
            switch ($rest_auth)
            {
                ...
                case 'oauth':
                    ...
                    {
                        // if condition for using rest controller in login api (exposing login api to public)
                        $this->oauth_verifier();
                        break;
                    } 
            }
    ...
    {
        if(isset($_SERVER['HTTP_USER']) && DATA_ENCRYPT){
            $_SERVER['HTTP_USER'] = base64_decode($_SERVER['HTTP_USER']);
            $_SERVER['HTTP_USER'] = str_replace(AES_KEY,'',$_SERVER['HTTP_USER']);
        }
        ...
        $_SERVER['HTTP_AUTHORIZATION']=isset($_SERVER['HTTP_AUTHORIZATION'])?$_SERVER['HTTP_AUTHORIZATION']:(isset($apache_headers['Authorization'])?$apache_headers['Authorization']:'');
    ...
    public function checkUploadFileType()
    {
        
        $flag = 0;

        $allowed_formats_temp = array(
            'text/plain',
            ...
            'application/rtf', 'application/x-rtf', 'text/richtext','application/octet-stream',
            'application/excel', 'application/vnd.ms-excel', 'application/x-excel', 'application/x-msexcel','application/zip','application/vnd.ms-office',
            'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            'application/vnd.ms-powerpoint','application/vnd.openxmlformats-officedocument.presentationml.presentation',
            'application/vnd.openxmlformats-officedocument.presentationml.presentation',
            'application/x-rar','application/x-zip-compressed','application/zip','message/rfc822','application/vnd.ms-outlook',
            ...
            'text/x-mailbox','video/mp4','video/quicktime','video/x-ms-wmv', 'video/x-ms-asf','video/x-matroska','video/x-flv','video/webm'
        ...
        $allowed_formats = array(
            ...
            'application/vnd.openxmlformats-officedocument.presentationml.presentation',
            'application/x-rar','application/octet-stream','application/x-zip-compressed','application/zip','message/rfc822','application/vnd.ms-outlook',
            'video/x-msvideo', 'video/msvideo', 'video/avi', 'application/x-troff-msvideo',
            'audio/mpeg', 'audio/mpg', 'audio/mpeg3', 'audio/mp3',
            'audio/x-wav', 'audio/wave', 'audio/wav',
            'text/x-mailbox','video/mp4' ,'video/quicktime','video/x-ms-wmv', 'video/x-ms-asf','video/x-matroska','video/x-flv','video/webm'
        ...
        $allowed_types = array(
            ...
            'xlsx',
            'ppt',
            'pptx','eml','msg','avi','mp3'.'wav','pst','mbox','f4v','mp4','mov','wmv','mkv','flv','webm'
        );
        ...
        if(isset($_FILES['customer']['name']['company_logo']) || isset($_FILES['file']['name']['profile_image']) || isset($_FILES['file']['name']['company_logo']) || isset($_FILES['file']['name']['branch_logo']) || isset($_FILES['file']['name']['country_flag']) || isset($_FILES['file']['name']['flag'])){
            $allowed_formats = $image_formats;
            $allowed_formats_temp = $image_formats_temp;
            $allowed_types = $image_types;

            if(isset($_FILES['customer'])){ $_FILES['file'] = $_FILES['customer']; }
        }
        ...
        if(isset($_FILES) && !empty($_FILES['file']))
        {
            ...
            $file_temps = array_values($_FILES['file']['tmp_name']);
            $file_temps_types=array();
            //echo '<pre>';print_r($_FILES);
            foreach($file_temps as $temp){
                $finfo = new finfo(FILEINFO_MIME_TYPE);
                $mime  = $finfo->file($temp);//echo '<pre>';print_r($mime);exit;
                $file_temps_types[]=$mime;
            }
            ...
            foreach($file_temps_types as $type){
                if (!in_array($type, $allowed_formats_temp)) {
                    $flag = 1; break;
                }
            }
            if($flag===0)
        ...
        if($flag==1)
        {
            $result = array('status'=>FALSE, 'error' => array('files' => $this->lang->line('invalid_format').' , Please upload valid documents '.join(', ',$allowed_types)), 'data'=>'');
            $this->response($result, REST_Controller::HTTP_OK);
        }
        else if($flag==2)
        {
            $result = array('status'=>FALSE, 'error' => array('files' => str_replace('%s',fileSizeFormat(IMAGE_UPLOAD_SIZE),$this->lang->line('max_upload_size'))), 'data'=>'');
            $this->response($result, REST_Controller::HTTP_OK);
        }
    }
